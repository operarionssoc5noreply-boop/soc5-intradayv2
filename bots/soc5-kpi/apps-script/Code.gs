/**
 * Google Apps Script implementation for the SOC5-KPI SeaTalk bot.
 *
 * Configure values in Project Settings > Script properties, then run
 * installScheduledSendTriggers() once from the Apps Script editor.
 */
...
const DEFAULTS = {
  ...
  WATCH_SNAPSHOT_PROPERTY: 'soc5_kpi_watch_range_snapshot',
  BOT_EXPECTED_SEND_INTERVAL_MINUTES: '0',
  BOT_LOGS_SHEET_NAME: 'bot_logs',
  BOT_DELAY_GRACE_MINUTES: '5',
};

const SCHEDULED_SEND_HOURS = [7, 12, 19, 0];

function sendKpiReport() {
  const cfg = loadConfig_();
  const spreadsheet = SpreadsheetApp.openById(cfg.GOOGLE_SPREADSHEET_ID);
  return sendKpiReportWithConfig_(cfg, spreadsheet);
}

function sendReportNow() {
  return sendKpiReport();
}

function installScheduledSendTriggers() {
  ScriptApp.getProjectTriggers()
    .filter(function(trigger) {
      return trigger.getHandlerFunction() === 'sendKpiReport';
    })
    .forEach(function(trigger) {
      ScriptApp.deleteTrigger(trigger);
    });

  SCHEDULED_SEND_HOURS.forEach(function(hour) {
    ScriptApp.newTrigger('sendKpiReport')
      .timeBased()
  ...
  console.log('Installed scheduled send triggers for sendKpiReport at 7AM, 12NN, 7PM, and 12MN.');
}

function installPollingTrigger() {
  return installScheduledSendTriggers();
}

function pollKpiWatchRange() {
  const lock = LockService.getScriptLock();
  if (!lock.tryLock(30000)) {
    console.warn('Skipping KPI poll because another poll is still running.');
  ...
  try {
    ...
    if (!previousSnapshot) {
      ...
      console.log('Initialized KPI watch snapshot for ' + cfg.GOOGLE_WATCH_RANGE + '. No report sent.');
      return { sent: false, changed: false, reason: 'initialized' };
    }

    if (previousSnapshot === snapshot) {
      console.log('No KPI watch range change detected in ' + cfg.GOOGLE_WATCH_RANGE + '.');
      return { sent: false, changed: false, reason: 'unchanged' };
    }

    props.setProperty(cfg.WATCH_SNAPSHOT_PROPERTY, snapshot);
    const result = sendKpiReportWithConfig_(cfg, spreadsheet);
...
function checkKpiSetup() {
  ...
  const summary = {
    ...
    scheduledSendTriggersInstalled: sendTriggers.length === SCHEDULED_SEND_HOURS.length,
    sendKpiReportTriggers: sendTriggers.length,
    scheduledSendHours: SCHEDULED_SEND_HOURS,
    ...
    kpiLastRunRange: cfg.GOOGLE_KPI_LAST_RUN_RANGE,
  ...
  if (sendTriggers.length !== SCHEDULED_SEND_HOURS.length) {
    throw new Error('Expected ' + SCHEDULED_SEND_HOURS.length + ' scheduled send triggers. Run installScheduledSendTriggers once from the Apps Script editor.');
  }
...
function sendKpiReportWithConfig_(cfg, spreadsheet) {
  const groupIds = readGroupIds_(spreadsheet, cfg);
  if (groupIds.length === 0) {
    throw new Error('No SeaTalk group IDs found in ' + cfg.GOOGLE_GROUP_IDS_RANGE);
  }

  const elements = buildKpiCardElements_(cfg, spreadsheet);
  const result = sendToGroups_(cfg, spreadsheet, groupIds, elements);
  if (result.sent === 0) {
    throw new Error('KPI report was not sent to any SeaTalk group. ' + result.errors.join(' | '));
  }
  if (result.errors.length > 0) {
    console.warn('KPI report sent to ' + result.sent + ' group(s), with skipped/failed groups: ' + result.errors.join(' | '));
  }

  return result;
}

function buildKpiCardElements_(cfg, spreadsheet) {
  const elements = [
    titleElement_(buildKpiTitle_(cfg)),
    descriptionElement_(buildKpiDescription_(cfg, spreadsheet)),
  ];
  const imageBase64 = buildKpiImage_(cfg, spreadsheet);
  if (imageBase64) {
...
function buildKpiDescription_(cfg, spreadsheet) {
  const kpiLastRunTime = readRangeDisplayValue_(spreadsheet, cfg.GOOGLE_KPI_LAST_RUN_RANGE);
  return 'KPI Last Run Time: ' + kpiLastRunTime;
}

function buildKpiImage_(cfg, spreadsheet) {
...
function exportReportPdfForRange_(spreadsheet, cfg, captureRange) {
  ...
  return response.getBlob().setName('soc5-kpi-report.pdf');
}
