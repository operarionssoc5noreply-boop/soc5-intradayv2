...
This bot polls `Summary Sheet (In progress)!AE6` every five minutes. When the displayed value changes, it waits 7 seconds for the sheet values to settle, then sends a SeaTalk text message followed by one image rendered from `Summary Sheet (In progress)!C2:V62`.
...
5. Run `sendReportNow()` once to authorize and test. It will skip sending if the watch range is `0`.
6. Run `installPollingTrigger()` once to start five-minute polling.
7. Run `initializeEnrouteAlertWatchSnapshot()` if you want the current watch value saved without sending a report.
